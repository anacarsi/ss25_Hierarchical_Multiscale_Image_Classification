...
def visualize_attention_heatmap(image, attention_weights, save_path=None):
    ...
    # Create a heatmap
    heatmap = plt.cm.jet(attention_weights_resized)[..., :3]  # Get RGB from the colormap
    heatmap = np.uint8(255 * heatmap)  # Scale to [0, 255]
    
    # Overlay the heatmap on the original image
    overlay = np.clip(0.5 * image.permute(1, 2, 0).numpy() + 0.5 * heatmap, 0, 255).astype(np.uint8)
